# GPU Optimization Guide for Nurse Supply Oracle

This guide covers GPU optimization for the Nurse Supply Oracle application using Hugging Face models with 4-bit quantization.
...
```bash
# Start the Hugging Face GPU server
...
- ✅ Full GPU acceleration with 4-bit quantization
- ✅ 80%+ GPU utilization
...
**Primary Model:**
- `mistralai/Mistral-7B-Instruct` (4-bit quantized)

**Alternative Models:**
- `microsoft/DialoGPT-large` (if authentication issues)
- `microsoft/DialoGPT-medium` (lower VRAM usage)
...
You can modify the model configuration in `scripts/chat_hf.py`:
